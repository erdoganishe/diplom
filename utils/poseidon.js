...
function PoseidonEx(nOuts, inputs, initialState) {
  ...
  for (let r = 0; r < nRoundsP; r++) {
    sigmaP[r] =
      r === 0 ? Sigma(mix[nRoundsF / 2 - 1][0]) : Sigma(mixS[r - 1][0]);
  ...
  for (let r = 0; r < nRoundsF / 2 - 1; r++) {
    for (let j = 0; j < t; j++) {
      sigmaF[nRoundsF / 2 + r][j] =
        r === 0
          ? Sigma(mixS[nRoundsP - 1][j])
          : Sigma(mix[nRoundsF / 2 + r - 1][j]);
    }
    input = sigmaF[nRoundsF / 2 + r];
    ark[nRoundsF / 2 + r + 1] = Ark(
      t,
      C,
      (nRoundsF / 2 + 1) * t + nRoundsP + r * t,
      input
    );
